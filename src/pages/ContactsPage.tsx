...
export default function ContactsPage() {
  ...
  return <div className="theme-executive">
    <Header title="Team Call&Chat" subtitle={`${contacts.length} people sorted alphabetically`} action={<button className="rounded-2xl bg-[#ff40b4] p-3 text-white shadow-lg" onClick={() => setAdding(true)}><Plus /></button>} />
    <section className="space-y-5 px-4 md:px-8">
      ...
      {contacts.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{contacts.map(c => <ContactCard key={c.id} contact={c} />)}</div> : <div className="stitch-card p-8 text-center text-[#c9badb]">No contacts match this search.</div>}
    </section>
  </div>;
}
